# Earthquake

基于 [Earth](https://github.com/halo-sigs/theme-earth) 二开的 Halo 主题。轻量、有态度。

面向个人博客、图库、瞬间和链接页，不追求大而全。

**预览：[风观墨迹](https://fg.ink)**

## 技术栈

Astro + Tailwind CSS + Alpine.js

## 安装

从 [Releases](https://github.com/mufengyian/theme-earthquake/releases) 下载最新 ZIP，在 Halo Console 主题管理中上传启用。

要求 Halo `>= 2.22.0`。

## 推荐插件

均非必需，按需安装：

| 插件 | 用途 |
| --- | --- |
| [链接管理](https://www.halo.run/store/apps/app-hfbQg) | 链接页 |
| [图库管理](https://www.halo.run/store/apps/app-BmQJW) | 图库 / 照片详情 |
| [瞬间](https://www.halo.run/store/apps/app-SnwWD) | 瞬间列表和详情 |
| [Shiki 代码高亮](https://www.halo.run/store/apps/app-kzloktzn) | 代码块高亮 |

## 主要能力

- 内置图片预览器（灯箱、缩略图栏、EXIF 面板、原图打开）
- 原生文章目录（左侧 / 文内 / 双目录）
- 多种文章卡片布局和显示项控制
- OKLCH 单变量配色系统，30+ 主题预设
- 明暗模式，支持访客手动切换（可选圆形扩散动画）
- 字体切换（系统 / Roboto / JetBrains Mono / MiSans / LXGW WenKai）
- 毛玻璃导航栏
- 侧边栏小组件排序
- 页脚版权、备案、社交链接配置

## 开发

```bash
git clone git@github.com:mufengyian/theme-earthquake.git
cd theme-earthquake
pnpm install
pnpm dev     # watch + rebuild
```

需要 Node.js `>= 22.13.0`，pnpm `11.8.0`。

常用命令：

```bash
pnpm run check    # 类型检查 + 静态构建
pnpm run build    # 构建 + 打包
pnpm run package  # 仅打包，产物在 dist/
pnpm run format   # 格式化
```

## 目录结构

```
src/
  assets/       样式、脚本、运行时工具
  components/   Astro 组件
  layouts/      页面布局
  pages/        Halo 模板页面
public/         静态资源
scripts/        构建辅助脚本
templates/      构建输出（勿手动修改）
dist/           打包输出
```

## 发布

1. 更新 `theme.yaml` 版本号
2. `pnpm run check`
3. 推送到 main，GitHub Actions 自动生成 Release

## 许可证

[GPL-3.0](./LICENSE)
